...
import { OpenApiUploadResult, ServiceUploadResult } from '../types/handleFileProcessing-types';
...
export const postOpenApi = async (
  ...
  const result: ServiceUploadResult = await generateAndUploadService(spec, userId);
...
export const postTests = async (
  ...
  const tempDir = join(tmpdir(), 'openapi-test-gen', userId, generateRandomString(10));
...
async function compileTypeScript(srcDirectory: string, outDirectory: string): Promise<void> {
...
export const generateAndUploadService = async (spec: OpenApi, userId: string) => {
  ...
  try {
    ...
    await uploadToS3(bucketNameForServices, s3Key, zipBuffer, 'application/zip');
...
const zipDirectory = async (sourceDir: string, outPath: string): Promise<Buffer> => {
